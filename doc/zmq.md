...
The OpayK Core daemon can be configured to act as a trusted "border
router", implementing the opayk wire protocol and relay, making
...
The ZeroMQ feature in OpayK Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building opaykd:
...
For instance:
    ...
    $ opaykd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/opaykd.tx.raw \
...
These options can also be provided in opayk.conf.
...
From the perspective of opaykd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into opaykd directly. Furthermore, no information is
...
using. OpayKd appends an up-counting sequence number to each
