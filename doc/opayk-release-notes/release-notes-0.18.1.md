OpayK Core version 0.18.1 is now available from:

  <https://download.opayk.org/opayk-0.18.1/>
...
`/Applications/OpayK-Qt` (on Mac) or `opaykd`/`opayk-qt` (on
Linux).
...
OpayK Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use OpayK Core on unsupported systems.

OpayK Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, OpayK Core does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](Https://github.com/opaykchain/blob/master/doc/zmq.md#usage)
  for details.
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:14911:14911` (this is an extra `:14911` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@opayk.org) the OpayK Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 OPAYK),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in OPAYK per kilobyte, not litoshi per byte.

Documentation
-------------

- A new short [document](Https://github.com/opaykchain/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](Https://github.com/opaykchain/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](Https://github.com/opaykchain/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](Https://github.com/opaykchain/blob/master/doc/opayk-conf.md)
  about the `opayk.conf` file describes how to use it to configure
  OpayK Core.

- A new document introduces OpayK Core's BIP174 [Partially-Signed
  OpayK Transactions
  (PSBT)](Https://github.com/opaykchain/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](Https://github.com/opaykchain/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent OpayK-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  OpayK Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](Https://github.com/opaykchain/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](Https://github.com/opaykchain/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when OpayK Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of OpayK Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](Https://github.com/opaykchain/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](Https://github.com/opaykchain/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart opaykd with the `-deprecatedrpc=generate`
...
- A new `opayk-wallet` tool is now distributed alongside OpayK
...
This section describes planned changes to OpayK Core that may affect
other OpayK software and services.

- Since version 0.16.0, OpayK Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with OpayK Core 0.20 (expected about
  a year after 0.18), OpayK Core will default to native segwit
  ...
  to bech32 addresses, and if the OpayK Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in OpayK Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any OpayK Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, OpayK Core
...
- A sub-project of OpayK Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with OpayK Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to OpayK Core's own implementation, although entropy
  gathered by OpayK Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves OpayK Core a
...
- On macOS, OpayK Core now opts out of application CPU throttling
...
- #14911 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
