...
CBlock TestChain100Setup::CreateAndProcessBlock(const std::vector<CMutableTransaction>& txns, const CScript& scriptPubKey)
{
    ...
    while (!CheckProofOfWork(block.GetPoWHash(Assert(m_node.chainman)->ActiveHeight(), [&chainman](uint32_t height){ return chainman->ActiveTip()->GetAncestor(height)->GetBlockHash(); }), block.nBits, chainparams.GetConsensus())) ++block.nNonce;
